...
seb0 = round((sigmasquare*((1/n)+((xbar**2)/sxx)))**0.5, 4)
...
for i in range(parts):
	...
	if option == 1:
		...
		print("Since we know that")
		print("\\\\\\hat{\\beta}_1 - t_{\\alpha/2, n-2}*S.E.(\\hat{\\beta}_1) < \\beta_1 < \\hat{\\beta}_1 + t_{\\alpha/2, n-2}*S.E.(\\hat{\\beta}_1)")
		print(f"\\\\{b1}- {t}*{seb1} < \\beta_1 < {b1} + {t}*{seb1}")
		print(f"\\\\{b1}- {round(t*seb1,4)} < \\beta_1 < {b1} + {round(t*seb1,4)}")
		print(f"\\\\ {round(b1 - (t*seb1),4)} < \\beta_1 < {round(b1 + (t*seb1),4)}")
	elif option == 16:
		ci = float(input("Confidence Interval (in %) = "))
		t = round(st.t.ppf(1-((1-(ci/100))/2), n-2), 4)
		print(f"\\\\\\alpha = {round(1-(ci/100), 4)}\ and\ d.f. = n-2 = {n-2}")
		...
		print("Since we know that")
		print("\\\\\\hat{\\beta}_0 - t_{\\alpha/2, n-2}*S.E.(\\hat{\\beta}_0) < \\beta_0 < \\hat{\\beta}_0 + t_{\\alpha/2, n-2}*S.E.(\\hat{\\beta}_0)")
